# Biorobotics Learning

My freshman-year journey toward becoming a strong Biorobotics engineer.

## About

I am developing skills in programming, electronics, CAD, robotics,
and engineering through coursework, hands-on projects, and research preparation.

## Focus Areas

- Programming
- Electronics
- CAD
- Robotics
- Engineering Fundamentals
- Research

## Projects

| Project | Skills | Status |
|---|---|---|
| Robot Motion Simulator | Python, Physics | 🟢 Completed |
| Sensor-Controlled Servo | Arduino, Electronics | 🟡 In Progress |

## Skills

### Programming
- Python
- C

### Hardware
- Arduino
- Sensors
- Motors
- Basic circuits

### CAD
- SolidWorks

### Tools
- Git / GitHub
- MATLAB

## Research Interests

- Biorobotics
- Human-Robot Interaction
- Rehabilitation Robotics
- Robotic Systems

## Current Goals

- Build strong programming fundamentals
- Develop hands-on electronics skills
- Learn CAD and mechanical design
- Gain research experience
- Contribute to undergraduate robotics research

## Learning Log

### August 2026
- Started Python
- Reviewed physics fundamentals
- Built Robot Motion Simulator
- Started learning Arduino

## Future Plans

- Build more robotics projects
- Develop sensor-based systems
- Join undergraduate research
- Explore robotics control and embedded systems
